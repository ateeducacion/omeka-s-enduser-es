...
Una vez que el coordinador te haya aceptado en el equipo, puedes ir al panel de control de Omeka S y hacer clic en el botón «Traducir» para comenzar a traducir.
...
La mayoría de las cadenas que hay que traducir en Omeka son sencillas y se pueden traducir directamente. Sin embargo, hay algunos casos especiales que deben tratarse con cuidado.

### Marcadores de posición
Algunas cadenas contienen texto que tiene el aspecto de `%s` o `%1$s`. Estos fragmentos de texto de aspecto extraño se denominan **marcadores de posición**. Los marcadores de posición se utilizan para permitir que Omeka inserte información variable, como el número de elementos de un sitio de Omeka, en una cadena traducida.
