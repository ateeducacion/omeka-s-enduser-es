...
Los proyectos de DataScribe configurados con visibilidad «pública» pueden ser vistos por cualquier usuario que haya iniciado sesión; los proyectos «privados» solo pueden ser vistos por usuarios con nivel de supervisor o administrador global. 

## Requisitos

Para poder exportar conjuntos de datos, el sistema debe tener permisos de escritura en el directorio `/files/assets` de su instalación de Omeka S.
...
Si tiene archivos TIFF o PDF, deberá convertirlos a uno de los formatos anteriores.
...
**Proyecto**: un conjunto de datos o un grupo de conjuntos de datos. Algunos usuarios de DataScribe pueden tener varios proyectos, mientras que otros solo tendrán uno. Cada proyecto tiene al menos un conjunto de datos. Se accede a todos los proyectos de DataScribe a través del panel de control.
...
**Los registros** son fragmentos individuales de datos de un elemento. Un único registro de DataScribe aparecerá como una fila cuando se exporten los datos transcritos. En términos de flujo de trabajo general, la transcripción se realiza a nivel de registro y la revisión a nivel de elemento. Sin embargo, los transcriptores pueden dejar notas y marcar registros individuales para que se les preste atención, incluso cuando el elemento completo no esté listo para ser revisado.
...
**Revisor**: cualquier usuario de Omeka S (de cualquier nivel) puede ser designado revisor de DataScribe. Este usuario puede revisar y aprobar registros y elementos creados por otros. 
...
![captura de pantalla del panel de control de DataScribe que muestra que el usuario no posee ni pertenece actualmente a ningún proyecto, con el botón Añadir un nuevo proyecto visible en la esquina superior derecha](modulesfiles/datascribe_buildproject2.png)

Todos los proyectos deben tener un nombre, que se configura en la pestaña «Configuración» (y se puede editar más tarde). También puedes, opcionalmente, añadir una descripción a tu proyecto. 
...
A continuación, debes añadir usuarios al proyecto. Si eres tanto el creador como el responsable del proyecto, este paso incluirá añadirte a ti mismo como usuario, incluso si no hay nadie más trabajando en el proyecto contigo. A pesar de ser el propietario del proyecto, no se le convierte automáticamente en usuario del proyecto. Esto significa que los proyectos pueden ser configurados por personas que gestionan la instalación de Omeka S, pero que no forman necesariamente parte de los equipos de proyecto individuales.

En la parte derecha de la pantalla aparecerá un menú que muestra todos los usuarios de la instalación de Omeka S. Puedes utilizar los elementos del menú alfabético (que se puede expandir haciendo clic en el triángulo) para explorar y encontrar usuarios, o puedes utilizar el campo «Filtrar usuarios» para buscar usuarios por nombre. Una vez que encuentres al usuario que deseas añadir, haz clic en su nombre para añadirlo al proyecto.
...
Una vez que hayas añadido un usuario al proyecto, debes establecer su rol en el proyecto. Todos los usuarios comienzan automáticamente como «Transcriptor» y hay un menú desplegable que se puede utilizar para cambiar su rol en el proyecto a «Revisor» si es necesario.

Añádete a ti mismo si tienes pensado utilizar tu cuenta de usuario actual para trabajar como transcriptor o revisor. Si eres supervisor o administrador global y solo tienes pensado administrar el proyecto, no es necesario que te añadas: siempre tendrás acceso a todos los proyectos de la instalación. 
...
|Fecha|Menús desplegables|Establecer año mínimo y máximo. Establecer año, mes y/o día por defecto|
|Fecha y hora|Menús desplegables|Establecer año mínimo y máximo. Establecer año, mes, día, hora, minuto y/o segundo por defecto|
|Número|Solo números. Decimales con punto, no con coma.|Establecer valor mínimo y máximo.|
...
|Texto|Entrada de texto de una sola línea|Establezca una longitud mínima o máxima|
...
| Tipo de anuncio  | Seleccionar o radio |  | Particular, empresa, organización, etc. |
...
Puedes crear un formulario para tu conjunto de datos al añadirlo o editarlo. En la pestaña del generador de formularios, utiliza el plan que has redactado como guía para añadir bloques de campos al formulario.
...
<div style="padding:68.15% 0 0 0;position:relative;"><iframe src="https://player.vimeo.com/video/1192402311?badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479" frameborder="0" allow="autoplay; pantalla completa; imagen en imagen; escritura en el portapapeles; contenido cifrado; compartir en la web" referrerpolicy="origen estricto en caso de origen cruzado" style="posición:absoluta;arriba:0;izquierda:0;ancho:100%;height:100%;" title="Exportar e importar formularios en DataScribe"></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>
...
Haga clic en el enlace para descargar el formulario. El título predeterminado de cualquier formulario exportado es «form_export»: si tiene previsto utilizar el formulario más de una vez, o va a exportar varios formularios, cambie el nombre del archivo al descargarlo.

#### Importar un formulario de conjunto de datos

**Solo** puedes importar un formulario existente al crear un nuevo conjunto de datos.

Recuerda que un conjunto de datos se basa en un conjunto de elementos, así que organiza los elementos pertinentes en un conjunto de elementos, crea un nuevo conjunto de datos basado en ese conjunto de elementos e importa el formulario durante ese proceso de creación. 
...
Cuando acceda para editar su nuevo conjunto de datos, debería ver ahora el formulario que ha importado. Desde aquí, también puede añadir, eliminar o modificar campos según sea necesario para este conjunto de datos específico.
...
Por ejemplo, es posible que haya iniciado un proyecto y haya creado un formulario para que coincida con un conjunto de elementos lleno de elementos que parecen similares, como páginas del censo. Luego descubre que hay un cambio en el formato de cada página en algún momento. Puede crear un segundo conjunto de elementos, mover los elementos posteriores del primer conjunto al segundo, copiar (exportar y volver a importar) su primer formulario y modificarlo para adaptarlo al segundo formato que ha descubierto, sincronizar el proyecto para mostrar los elementos actuales en cada conjunto de elementos y continuar transcribiendo los elementos posteriores. 

Una vez que hayas creado el conjunto de datos, se te redirigirá a la página de exploración del conjunto de datos. Debería aparecer un mensaje en el área de trabajo principal que dice «No se han encontrado elementos. Sincroniza este conjunto de datos».

Para sincronizar su conjunto de datos, tiene dos opciones.

En primer lugar, puede hacer clic en la frase «Sincronizar este conjunto de datos», situada en el centro del panel de control del conjunto de datos, y luego volver a hacer clic en el panel de la derecha para confirmar la sincronización. La primera sincronización rellenará el conjunto de datos con elementos del conjunto de elementos de origen.
...
Se le redirigirá a la página Registros. Los registros de las transcripciones aparecerán en el centro de la pantalla. En la parte derecha de la pantalla se encuentran las «Acciones del elemento» con el estado de bloqueo, lo que significa que solo el revisor y la persona a la que está bloqueado pueden editarlo. En la esquina superior derecha, haz clic en «Añadir nuevo registro».

### La pantalla de transcripción

En la parte central derecha se encuentra el formulario que debe rellenar según la imagen de la parte central izquierda de la pantalla. Un asterisco indica que ese campo es obligatorio.
...
En la parte superior izquierda hay tres botones para ajustar la pantalla y mejorar el flujo de trabajo. «Habilitar modo de enfoque» te permite ocultar los paneles laterales mientras transcribes. El «Modo de entrada rápida» elimina las casillas de verificación adicionales del formulario. El tercer botón modifica el diseño: puede elegir la vista horizontal (imagen junto al formulario) o vertical (imagen encima del formulario).
...
Los revisores comprueban las transcripciones después de que los transcriptores hayan completado la transcripción de un elemento de DataScribe y la hayan enviado para su revisión. Podrás aprobar la transcripción o devolverla al transcriptor para que la corrija.

Hay varias formas de encontrar los registros para su revisión. La forma más fácil es buscar en Mis proyectos y seleccionar Todos los elementos que necesitan revisión en el conjunto de datos que desees revisar. Una vez que llegues a la página del conjunto de datos, puedes filtrar los elementos que necesitan revisión.
...
Puedes ordenar estos elementos por título, la fecha en que se enviaron para su revisión, el historial de revisión del elemento y si el elemento tiene prioridad o no.

Una vez que encuentres el elemento que debes revisar, selecciona el título del elemento para ver la página de registros. Las directrices de su proyecto deben determinar la relación entre los registros y los elementos.

Un icono de exclamación roja junto a cualquier número de registro indica un problema con los campos obligatorios de los registros. Otros problemas de transcripción, como errores ortográficos, campos omitidos accidentalmente o secciones ilegibles, requieren que revise todo el formulario.

Es posible desplazarse por la tabla de registros para ver las entradas sin la imagen del elemento, pero esto depende del nivel de revisión que requieran las directrices del proyecto.
...
Puedes marcar «Necesita revisión» y dejar notas en el registro. Si estás transcribiendo varios registros por elemento, las acciones de registro en el lado derecho permitirán a los revisores marcar el registro específico.

Puedes dejar notas a nivel de elemento en «Acciones del elemento». Desde aquí, haz clic en «Guardar… y volver a los registros».

Establecer un estado de revisión para el elemento es el último paso. El elemento debe establecerse como «aprobado» o «no aprobado». En el estado de revisión, seleccione «Marcar como aprobado» o «Marcar como no aprobado».

Si el elemento no está aprobado, debe dejar una nota en el elemento o registro para indicar al transcriptor qué problemas debe corregir y que marque solo las áreas que necesitan revisión cuando se vuelva a enviar el elemento.

Haga clic en «guardar» para asegurarse de que se guarden los cambios.
...
![Panel de metadatos del conjunto de datos con flechas azules que señalan las opciones de último conjunto de datos validado, último conjunto de datos exportado y descargar conjunto de datos](modulesfiles/datascribe_exportData.png)

Para acceder a la exportación, utiliza el enlace «Haz clic para descargar el conjunto de datos (CSV)» situado en la parte inferior de la barra lateral de metadatos del conjunto de datos. Si haces clic en el enlace, se abrirán los datos CSV en la ventana de tu navegador. Para guardar el archivo, haz clic con el botón derecho o pulsa Control y haz clic, y utiliza el menú de tu navegador para guardar el archivo vinculado en tu ordenador.
