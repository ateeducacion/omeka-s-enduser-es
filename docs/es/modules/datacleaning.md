...
Los cambios realizados por el módulo de limpieza de datos no se pueden deshacer fácilmente y pueden ser destructivos. Debido a la gran potencia del módulo, solo pueden utilizarlo los administradores globales. 
...
En la sección de «Limpieza de datos» dentro de «Módulos», haz clic en el botón «Preparar nueva auditoría» situado en la esquina superior derecha de la página. 
...
- **Tipo de recurso**: selecciona el tipo de recurso que deseas auditar en el menú desplegable. Las opciones son elementos, conjuntos de elementos o archivos multimedia.
...
- **Propiedad de destino**: Selecciona la propiedad en la que deseas que se almacenen las correcciones. No es necesario seleccionar una propiedad de destino si es la misma que la propiedad anterior. Este menú desplegable cargará todas las propiedades de todos los vocabularios de su instalación. Cuando está abierto, la parte superior del menú funciona como un campo de búsqueda que le permite localizar una propiedad más rápidamente. 

- **Columna de auditoría de destino**: Selecciona la columna en la que se almacenarán las correcciones. No es necesario seleccionar un destino si es el mismo que la columna de auditoría anterior. Las opciones son `value`, `uri` o `value_resource_id`. 
...
Una vez que hayas introducido la información en todos los campos pertinentes, haz clic en el botón «Enviar» situado en la esquina superior derecha. 
...
- **Validar todo**: Comprueba todos los cambios propuestos para asegurarse de que son posibles dentro de los límites de la columna y el tipo de datos actuales; es decir, no se puede establecer un `value_resource_id` como una cadena de texto literal porque el tipo de datos requiere un número. La validación solo comprueba que la entrada sea estructuralmente posible, no que la modificación introducida exista; es decir, se podría cambiar un `value_resource_id` por un número que no se correspondiera con un ID real de Omeka S.
...
- Seleccione los criterios (expanda las flechas para obtener más información):
