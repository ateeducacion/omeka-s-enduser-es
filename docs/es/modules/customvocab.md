...
Por ejemplo, quizá quieras crear una lista específica de tu institución con ubicaciones que se correspondan con diferentes colecciones de tu campus, o una lista controlada de personas o lugares relacionados con tus fondos. Esto puede ayudar a reducir los errores tipográficos y las variaciones en los nombres, y te permite ofrecer [navegación por metadatos](../modules/metadatabrowse.md) para más campos.

«Vocabulario personalizado» está disponible para los usuarios con el [rol de editor o superior](../admin/users.md).

## Crear un vocabulario personalizado

Una vez que hayas instalado y activado el módulo, ve a la sección «Vocabulario personalizado» en el menú de navegación de la izquierda, bajo «Módulos».
...
!!! nota
  Ten en cuenta que los términos o URI introducidos manualmente no tienen por qué ser únicos en el momento de su introducción; el módulo solo conservará una lista de entradas únicas al guardarlas. Si introduces URI idénticos con etiquetas diferentes, solo se conservará la última entrada y se ignorarán las etiquetas anteriores.

## Gestionar vocabularios personalizados

Una vez que hayas creado al menos un vocabulario, la pantalla «Vocabulario personalizado» mostrará una tabla con tus vocabularios existentes. La tabla muestra la etiqueta, los botones para editar, eliminar y mostrar información (un icono de tres puntos), y la cuenta de usuario propietaria del vocabulario. 

También hay un botón para «Importar» un vocabulario a partir de un archivo, situado en la esquina superior derecha. La importación añadirá una nueva entrada a la tabla. Si intentas actualizar un vocabulario existente en tu instalación, no utilices el botón «Importar desde archivo»; «Actualiza» el vocabulario desde su entrada en la tabla.
...
Al hacer clic en los tres puntos, se mostrará el idioma de un vocabulario, así como una lista completa de sus términos. Hay dos botones que te permiten «Exportar» un vocabulario, que luego se puede compartir con otras instalaciones de Omeka, o «Actualizar» el vocabulario existente a partir de un archivo. 
...
Al editar un vocabulario personalizado, puedes cambiar la etiqueta, el idioma o los términos.
...
Ve a la pantalla de plantillas de recursos y añade una nueva plantilla o edita una ya existente. Una vez en la pantalla de edición de la plantilla:
...
1. Desplázate por el menú desplegable y selecciona el vocabulario que quieras utilizar.
...
Cuando se utilice esta plantilla de recurso para un elemento, un medio o un conjunto de elementos, la propiedad designada se cargará siempre como un menú desplegable con los valores del vocabulario personalizado. Recuerda que el vocabulario personalizado **requiere** que los usuarios lo utilicen. Si solo deseas ofrecer el vocabulario personalizado como una opción, añade un segundo tipo de datos o varios tipos de datos que ofrezcan alternativas, como un campo de texto sin formato. 

En la siguiente imagen, se carga la plantilla de recurso modificada anteriormente para un nuevo elemento. El campo «Cobertura espacial» muestra el mensaje «Selecciona a continuación» y unas flechas que indican que el campo de la propiedad es un menú desplegable.

![como se ha descrito](../modules/modulesfiles/customVocab_item1.png)
...
También puedes aplicar vocabularios personalizados como tipos de datos a valores existentes en recursos ya creados, independientemente de si utilizan o no una plantilla de recurso. 
...
![La opción «Convertir tipo de datos» en la pantalla de edición por lotes, que muestra «Creador» como primera selección y el menú desplegable abierto para mostrar los vocabularios de «Vocabulario personalizado» en la segunda selección.](../modules/modulesfiles/customVocab_batchEdit.png)

Esta configuración del tipo de datos se aplicará a los valores existentes en la propiedad, siempre que sea posible. Los valores de texto, si coinciden exactamente con las entradas de una lista de vocabulario personalizado de texto, funcionarán. Las URI, si coinciden exactamente con las entradas de una lista de vocabulario personalizado de URI, también funcionarán. No se pueden asignar URI mediante etiquetas coincidentes. 
...
Si, por ejemplo, tienes valores URI con formato de texto sin formato en elementos existentes y deseas convertirlos para que coincidan con un vocabulario personalizado de URIs, puedes hacerlo mediante un proceso en dos fases. En primer lugar, convierte los valores de las propiedades de los elementos de texto a URI utilizando el mismo proceso de edición por lotes; a continuación, convierte esas URI al vocabulario personalizado que hayas elegido. 