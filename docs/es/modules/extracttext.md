...
Si tienes más de un archivo adjunto a un elemento, el texto extraído acumulado aparecerá en un único campo a nivel del elemento, en el orden en que se hayan añadido los archivos. El texto extraído de los archivos nuevos se añadirá al final del campo.
...
- Extraer por lotes texto de todos los archivos multimedia asociados a varios elementos, desde la página de edición por lotes
- Extraer por lotes texto de una [Importación CSV](../modules/csvimport.md) que contenga archivos multimedia subidos.
...
Puedes borrar o volver a ejecutar manualmente el texto extraído editando el elemento o el archivo multimedia. Para cambiar la salida automática, vaya a la pestaña «Extraer texto» de un elemento o de un archivo multimedia específico.

![Un elemento en modo de edición, en la pestaña «Extraer texto», mostrando las opciones para borrar o actualizar el texto.](../modules/modulesfiles/extracttext_item_edit.png)

Puede borrar todo el texto reconocido que se encuentra actualmente en el campo `extracttext:extracted_text`, o puede actualizarlo. A nivel de elemento, esto extraerá texto nuevo de todos los archivos multimedia del elemento, en orden, y lo mostrará en el campo como un único valor. 
...
Puede borrar o actualizar el texto de más de un elemento a la vez (o de más de un archivo multimedia, seleccionado manualmente). «Extraer texto» aparece como un nuevo campo en la pantalla de edición por lotes. Esto funcionará únicamente como un trabajo en primer plano, y deberá esperar a que la operación se complete. Los extractores en segundo plano no se ejecutarán desde una operación de edición por lotes.
...
[La importación CSV](../modules/csvimport.md), al añadir nuevos archivos multimedia a su instalación de Omeka, incluirá un proceso de extracción de texto, que abarca todos los extractores en primer plano y en segundo plano, en cada uno de los archivos. 
...
Puedes modificar el funcionamiento de este módulo editando los valores del archivo `config/local.config.php` de tu instalación. Utiliza la configuración «`extract_text`/`options`». Por ejemplo, si deseas omitir siempre la primera página de los PDF, añade lo siguiente:
...
También puede consultar el [manual de Tesseract](https://github.com/tesseract-ocr/tesseract/blob/main/doc/tesseract.1.asc){target=_blank} para obtener más información.

### Extraer en segundo plano

Puedes configurar un extractor para que nunca se ejecute en primer plano utilizando el ajuste «`extract_text`/`background_only`» en tu archivo de configuración local (`config/local.config.php`). 

Por ejemplo, si quieres configurar el extractor pdftotext para que se ejecute solo en segundo plano, añade lo siguiente:
