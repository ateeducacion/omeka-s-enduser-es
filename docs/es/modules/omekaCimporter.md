...
Este módulo solo importa elementos y sus metadatos (incluidas las etiquetas), archivos adjuntos y colecciones (como conjuntos de elementos). *No* importará contenido de «Página simple» ni de «Exhibit Builder».
...
Una vez instalado, el módulo Omeka Classic Importer debería aparecer en la parte inferior del menú de navegación izquierdo del panel de administración principal. Al seleccionarlo, aparecerán las opciones del submenú **Importar** e **Importaciones anteriores**. 
...
* **Clave API de Omeka Classic**: Este campo puede dejarse en blanco o rellenarse, dependiendo de si dispones de una clave API para el sitio desde el que intentas importar datos.  
* **Comentario**: Puedes añadir un comentario como «API de Jane Doe, sin clave» para ayudar a identificar la importación específica al consultar importaciones anteriores.  
* **Importar a**: Esta opción te permite seleccionar uno o varios conjuntos de elementos en los que importar los elementos. Ten en cuenta que solo puedes importar a conjuntos de elementos de los que seas propietario. No puedes crear un nuevo conjunto de elementos desde esta página. Quizás te interese importar colecciones en lugar de, o además de, importar a un conjunto de elementos aquí.
* **Por página**: Introduce un número para limitar el número de registros recuperados por solicitud. Útil para sitios que puedan tener un gran número de elementos.
...
Las columnas de la tabla son **Elemento de Omeka Classic**, **Propiedades asignadas** y una casilla de selección para **Conservar el HTML como archivo multimedia**. Cualquier campo que contenga código HTML que desees conservar en tu instalación de Omeka S, como saltos de línea y estilo de texto en el campo de descripción, puede importarse como un archivo HTML adjunto al elemento. El campo se importará como texto sin formato a los metadatos y con su formato como archivo multimedia.

![La pestaña «Asignar a propiedades de Omeka S», que muestra un botón «Borrar valores predeterminados» en la parte superior y una tabla de propiedades de Dublin Core a continuación. Cada línea de la tabla tiene una propiedad correspondiente rellenada automáticamente, un icono de papelera y una casilla de verificación para «Conservar HTML como archivo multimedia». El panel de la derecha está abierto y muestra todos los vocabularios con sus propiedades.](../modules/modulesfiles/oCi_mapprop.png)

El primer conjunto de la tabla es Dublin Core, seguido de los metadatos de tipo de elemento. La importación también incluirá cualquier conjunto de elementos heredados o adicionales (por ejemplo, «Omeka Legacy File»), o elementos creados por complementos de Classic (como una sección «Texto de PDF» para texto extraído). Estos elementos basados en complementos pueden asignarse automáticamente si tienes instalado el módulo equivalente de Omeka S (como «Extract Text» en este caso). 

Para asignar:

1. Selecciona el elemento o el tipo de elemento en la tabla haciendo clic en su fila o etiqueta.
1. En el panel de la derecha, haz clic o busca la propiedad de S a la que deseas asignar tu elemento de Classic. 
...
* **ID de tarea**: Un valor numérico asignado a cada importación de API específica. También puedes ver la tarea en la [pestaña «Tareas» del panel de administración](../admin/jobs.md).  
* **Comentario**: Si introdujiste un comentario al configurar la importación, ese comentario aparecerá aquí.
...
* **Estado**: Será «en curso», «completada» o «deshacida».  
