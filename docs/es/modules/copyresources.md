...
![La tabla de páginas en el panel de administración, que muestra botones de «copiar» junto a cada página de la tabla. El panel lateral derecho está abierto en la página «Bienvenida» y muestra un menú desplegable de «Visibilidad» encima del botón «Confirmar copia».](modulesfiles/copyResources_pages.png)
...
Los usuarios pueden copiar recursos que no les pertenezcan. Cuando se copia un recurso, el usuario que ha realizado la copia se convierte en su propietario; el recurso no conservará al propietario del original. 
...
Otros módulos cuyos datos pueden ser copiados por «Copy Resources», como «Navegación por metadatos», no tienen dependencias. 

## Copia de recursos

![La tabla de elementos en el panel de administración, que muestra botones de «copiar» junto a cada elemento de la tabla y un mensaje de confirmación en el panel lateral derecho.](modulesfiles/copyResources_items.png)
...
Al hacer clic en el botón «copiar», aparecerá un mensaje de confirmación en el panel de la derecha. Esta ventana de confirmación te permitirá configurar la visibilidad del recurso duplicado; las opciones son «Igual que el original», «Privado» o «Público». 
...
![La página de vista del sitio, después de que se haya copiado correctamente un sitio.](modulesfiles/copyResources_sitesCopied.png)

En los casos en que otros módulos introduzcan datos específicos del sitio (como «Navegación por metadatos», «Compartir», «Visualización de datos», «Recopilación», «Navegación por facetas», «Bloque de carrusel de elementos», etc.), esta información se copiará, [siempre que el módulo correspondiente se haya actualizado donde sea aplicable (véase la sección «Requisitos» más arriba)](#requirements). 
...
Copiar un sitio completo es un proceso complicado; los sitios contienen una gran variedad de datos, incluidos los procedentes de módulos y temas. Copiar un sitio puede dar lugar a errores que deben corregirse antes de que el sitio pueda hacerse público, y es posible que haya que recrear manualmente algunos datos a partir del sitio original antes de poder obtener una copia perfecta. 

Algunos problemas con los que te puedes encontrar:

- Un sitio cuya navegación tenga entradas de página que faltan puede provocar errores hasta que se eliminen o se corrijan las páginas que faltan. Esto puede incluir problemas al cambiar el tema, el nombre o el slug del sitio. Si ves los errores «Navegación no válida: el enlace de la página carece de ID de página» o «Navegación no válida: datos de enlace no válidos», comprueba tu navegación en busca de entradas que muestren «[Página ausente]».
- Algunos sitios que utilicen el módulo [Faceted Browse](facetedbrowse.md) pueden mostrar errores si el usuario que duplica el sitio no tiene permisos para utilizar Faceted Browse (los administradores globales y los supervisores sí pueden, pero los editores no). 
