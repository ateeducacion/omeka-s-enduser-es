...
Algunas propiedades inversas son evidentes (por ejemplo, «Tiene parte» de Dublin Core es la inversa prevista de «Es parte de»), pero otras son flexibles en función del tipo de elementos de la instalación. Por lo tanto, las relaciones de propiedades inversas de tu instalación deben especificarse manualmente.

Los enlaces inversos creados de esta forma pueden ahorrar tiempo en la introducción de datos y resultar útiles en [visualizaciones de datos](datavisualization.md), en la navegación avanzada o por facetas, y en la [importación por lotes](csvimport.md) de relaciones entre recursos.
...
- Dublin Core (`dcterms:`):
  ...
  - Referencia y es referenciado por
...
![Página de visualización del elemento guardado, en la que se muestra un elemento vinculado en la propiedad «Relación». Hay una flecha azul que apunta hacia él. La miniatura del elemento se muestra a la izquierda del título del mismo, con un icono de cubo a la derecha que indica que se trata de un tipo de datos de recurso vinculado.](modulesfiles/inverseProperties_itemShow.png)
...
![La página de visualización del elemento correspondiente al recurso que se ha vinculado. Su propiedad «Relación» también se ha rellenado, con un enlace que remite al primer elemento. Se muestran su miniatura y su título, también con un icono de cubo.](modulesfiles/inverseProperties_linkedProperty.png)
...
![El formulario de edición en masa muestra una entrada para «Propiedades inversas», con una casilla de selección para generar los emparejamientos IR.](modulesfiles/inverseProperties_batchedit.png)
