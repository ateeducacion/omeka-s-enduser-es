...
Puedes marcar estas casillas al instalar Fedora Connector por primera vez o más adelante mediante el botón «Configurar» de la lista de [Módulos](../modules/index.md).
...
* Casilla de selección **Ignorar contenedor principal**: márcala si solo quieres importar los descendientes del contenedor principal indicado en la URI del contenedor, no el contenedor principal en sí (opcional).
* Casilla de verificación **Importar solo descendientes directos**: márquela si solo desea importar los descendientes directos del contenedor indicado en la URI del contenedor (sin descendientes de los descendientes). Si no se marca, se importarán de forma recursiva todos los recursos que se encuentren por debajo del contenedor (opcional).
