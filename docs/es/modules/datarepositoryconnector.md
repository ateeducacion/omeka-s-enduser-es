...
El conector de repositorios de datos añade una entrada en el menú «Módulos» de la izquierda. Al hacer clic en el nombre del módulo o en el icono del signo más situado a la izquierda, el menú se expandirá para mostrar los enlaces a las cuatro opciones de importación. El módulo se abre por defecto en la página «Importaciones anteriores».
...
+ **Límite**: El número máximo de resultados que se recuperarán en un solo lote. Si detectas errores o datos que faltan, prueba a reducir este número. Aumentarlo podría acelerar las importaciones. (Este campo es obligatorio. El valor por defecto es 100.)
...
+ **Sitios**: Los sitios en los que se importarán los elementos. Es posible que aquí aparezca un sitio por defecto.
...
+ **Identificador de Dataverse**: El identificador del Dataverse desde el que se va a importar. Si se deja en blanco, se importarán todos los conjuntos de datos incluidos en la **URL principal de Dataverse**, lo que podría suponer un volumen considerable.
...
+ **ID de la comunidad de Zenodo**: La cadena corta que identifica de qué comunidad de Zenodo se va a realizar la importación. Se encuentra en la URL después de «https://zenodo.org/communities/» y antes de cualquier parámetro de búsqueda. (Este campo es obligatorio.)
+ **Formato de metadatos**: El formato de metadatos que se va a exportar desde Zenodo. La única opción para Zenodo es «oai_dc». El formato debe existir como [vocabulario](../content/vocabularies.md) en tu instancia de Omeka antes de la importación; puedes obtenerlo de [purl.org](http://purl.org/dc/elements/1.1/){target=_blank}. (Este campo es obligatorio.)
...
!!! Nota
  ¿Tus tareas se inician pero no se completan? Es posible que tengas que [configurar la ruta de PHP](../configuration.md#php-path) para que tu sistema pueda ejecutar el proceso en segundo plano necesario para crear los elementos.
...
Si una importación concreta se ha deshecho o se ha vuelto a ejecutar, la columna «Estado» lo indicará. Puede ver un subconjunto de los elementos afectados de cada tarea haciendo clic en el enlace «Total» situado debajo de la columna «Elementos».
