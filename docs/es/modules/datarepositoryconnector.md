...
Este módulo está disponible para usuarios con los niveles de «Administrador global» y «Supervisor».
...
+ **Límite**: El número máximo de resultados que se pueden recuperar en un solo lote. Si observas errores o datos que faltan, prueba a reducir este número. Aumentarlo podría acelerar las importaciones. (Este campo es obligatorio. El valor predeterminado es 100.)
+  **Importación de prueba**: Si se marca, el conector importará ÚNICAMENTE el número de resultados indicado en el campo «Límite» anterior. Esto resulta útil para obtener una vista previa de la correspondencia de metadatos entre la fuente y Omeka S, y para garantizar que la conexión funciona.
...
!!! Nota
  Volver a ejecutar una importación borrará cualquier modificación de metadatos que hayas realizado en los elementos desde la importación original.
