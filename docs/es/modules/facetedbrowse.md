...
Una vez activado el módulo de navegación por facetas, aparecerá una pestaña para la navegación por facetas en el menú contextual de cada sitio. Al hacer clic en esta pestaña, accederás a una lista de todas tus páginas de navegación por facetas para ese sitio.
...
El **título de la página** es obligatorio y se mostrará en las pestañas del navegador y se enviará en los metadatos de la página. Puedes establecer una etiqueta independiente en la navegación del sitio. La mayoría de los temas no mostrarán este título de forma visible en la página. Cuando se utiliza una sola categoría, su título aparecerá en la página; cuando se configuren dos o más categorías, el encabezado «Explorar» se mostrará encima de los enlaces de categorías en la barra lateral (consulta [la captura de pantalla siguiente](#multiple-categories-on-one-page) para ver cómo se mostrará). 
...
Por ejemplo, quizá quieras añadir una página general de «Explorar elementos» a tu navegación y ofrecer en ella una categoría para navegar solo por imágenes, otra categoría para navegar solo por eventos y una última categoría que permita a los usuarios explorar todos los elementos del sitio, incluidas imágenes y eventos. O bien, podría crear páginas de navegación por facetas independientes para añadirlas a su navegación —una para cada clase de elemento— y, dentro de cada página, ofrecer categorías para restringir aún más los recursos según otro valor. 
...
Tras configurar la categoría, puedes crear facetas y establecer columnas para la visualización de la navegación. Una vez que hayas terminado de crear tus facetas y de configurar las columnas de visualización, guarda la categoría.
...
Las facetas funcionan dentro de las categorías que haya creado. Puedes tener una o más facetas para cada categoría. Estas son las opciones que utilizarán los visitantes del sitio para filtrar la lista de elementos.
...
Una vez seleccionado el tipo, haz clic en el botón «Añadir». Se abrirá un panel lateral a la derecha de la ventana del navegador con opciones para configurar la faceta. Los nombres de las facetas son siempre obligatorios y se mostrarán en la interfaz pública. 
...
Para los tipos de selección «Única (lista)» y «Múltiple (lista)», los creadores de páginas pueden optar por truncar los valores de la lista que ven los visitantes del sitio, introduciendo un número en la opción «Truncar valores». Si se deja el campo en blanco, se mostrarán todos los valores. Al introducir un número, solo se mostrarán ese número de facetas, en orden, con un enlace «Ver más (X)» que muestra el número de facetas adicionales. 
...
Utiliza el menú desplegable para seleccionar qué propiedad se va a utilizar para la faceta. Por ejemplo, quizá quieras seleccionar el campo de descripción y permitir a los usuarios buscar dentro de esos textos descriptivos. O quizá quieras seleccionar el campo de materia y permitir a los usuarios ver todos los valores controlados que estás utilizando como encabezados de materia. Puede dejar este campo en blanco para ofrecer opciones de búsqueda o navegación en todas las propiedades de los recursos.
...
- Única (menú desplegable). Los visitantes pueden seleccionar solo una opción; todas las opciones se muestran en un menú desplegable.
...
- «Contiene»: los visitantes pueden elegir un valor que aparezca en cualquier parte del valor de la propiedad.
- «No contiene»: Los visitantes pueden seleccionar un valor que no aparezca en ninguna parte del valor de la propiedad.
- «Es un recurso con ID»: Los visitantes seleccionarán un recurso que se incluirá. 
	- Si a continuación se selecciona «Añadir todos los valores disponibles», los visitantes verán una lista de recursos que se [utilizan como valores en otros elementos (es decir, recursos vinculados)](../content/items.md#linked-resources). Se puede emplear esta opción sin seleccionar ninguna propiedad; en ese caso, se incluirán todos los valores de recursos vinculados. Ten en cuenta que los ID no se mostrarán en la interfaz pública, aunque sí se muestran en el panel de administración.
-  «No es un recurso con ID»: Los visitantes seleccionarán un recurso que se excluirá. 
  - Si a continuación se selecciona «Añadir todos los valores disponibles», los visitantes verán una lista de recursos que se utilizan como valores en otros elementos. Puede utilizar esta opción sin seleccionar ninguna propiedad; en ese caso, se incluirán todos los valores de los recursos vinculados. Tenga en cuenta que los ID no aparecerán en la interfaz pública, aunque sí se muestran en el panel de administración.
...
En el campo de texto «Seleccionar tipo», puede establecer una propiedad específica para la búsqueda o dejarlo en blanco para buscar en todas las propiedades. La búsqueda de texto exacto no distingue entre mayúsculas y minúsculas. Las opciones son:
...
No se puede dejar el campo de consulta en blanco. Es posible que desees proporcionar varios tipos de consultas de selección para el mismo campo, con el fin de ofrecer más detalle a los visitantes del sitio.

Para los tipos de selección Única (lista) y Múltiple (lista), los creadores de páginas pueden optar por truncar los valores disponibles en esta lista que es visible para el visitante del sitio, estableciendo un número en la opción «Truncar valores». Si se deja el campo en blanco, se mostrarán todos los valores. Si introduces un número, solo se mostrarán ese número de facetas, en orden, junto con un enlace «Ver más (X)» que indicará el número de facetas adicionales.
...
Por ejemplo, es posible que desees cargar todos los valores de la propiedad «Tema» y permitir a los usuarios explorar los elementos utilizando los encabezados temáticos actualmente en uso. Si seleccionas «Mostrar todos los valores disponibles», verás una lista de los temas actualmente en uso, ordenados de más frecuentes a menos. Ten en cuenta que quizá te interese limpiar tus datos y consolidar valores similares, o corregir errores tipográficos y variaciones, para que la navegación por facetas resulte más útil. Puedes utilizar el [módulo Value Suggest](../modules/valuesuggest.md) junto con la navegación por facetas para ver y limpiar datos desordenados.
...
Configure el tipo de selección para la faceta de navegación. En la opción «Múltiple (lista)», las selecciones múltiples realizadas por los visitantes del sitio ampliarán (Opción 1 O Opción 2) los resultados de la búsqueda. 
...
Puedes configurar la información que se muestra sobre los resultados añadiendo columnas de metadatos a la visualización de tu búsqueda por facetas. Esto convertirá la visualización en una tabla con una fila para cada recurso de los resultados. Las columnas se configuran por categoría. Dependiendo de si su página tiene una o varias categorías, la visualización de la página inicial puede variar.
...
Cada columna debe tener un título, que se mostrará en el encabezado de la tabla. También puedes impedir que los visitantes del sitio puedan ordenar los datos de cada columna. A continuación se describen otros ajustes.
...
Las vistas públicas de las páginas de navegación por facetas incluyen la página base, donde puede haber una o más categorías en las que hacer clic, y luego las páginas específicas de cada categoría, que muestran las columnas elegidas para esa vista. Las facetas de navegación se mostrarán en una lista si no se ha seleccionado ninguna columna para su visualización —no se incluirán encabezados de columna—, pero los elementos de la página se pueden ordenar como de costumbre.
...
Dependiendo de la configuración de columnas de cada categoría, es posible que haya más columnas de las que cabe en la ventana del navegador del usuario. En ese caso, solo verá el número de columnas que quepan en la página sin desbordarse, y aparecerán unas flechas a la izquierda y a la derecha, situadas encima de la esquina superior derecha de la tabla, para desplazar las columnas ocultas y poder verlas. Ten en cuenta que el texto de las columnas que no se muestran en la página no aparecerá si el usuario realiza una búsqueda de texto en la página.
...
* Límite: elige el número máximo de elementos que se mostrarán de los resultados predeterminados de la categoría de FB. Consulta la propia página de Facebook para ver qué elementos se mostrarán según su configuración actual. 
...
![Una búsqueda de texto de «william» ha cargado la página de «Búsqueda por facetas», y «william» también aparece en la faceta de texto completo de la barra lateral.](modulesfiles/FacetedBrowse_sitewide.png)
