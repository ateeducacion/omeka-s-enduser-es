...
![Página principal de la instalación de «Stackable Sandbox» en la que se muestran siete sitios, tres de los cuales tienen resúmenes.](files/frontpage-basic.png)

Si deseas que esta página de índice quede oculta a los visitantes del sitio, un administrador global puede seleccionar un sitio público de tu instalación al que se redirigirá a los visitantes cuando accedan a tu URL base (de `yourdomain.org/omekas/` a `yourdomain.org/omekas/s/yourmainsite/`). Para ello:
