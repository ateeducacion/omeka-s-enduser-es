...
Los roles de usuario establecidos a nivel de instalación son independientes de los [roles de usuario específicos del sitio](../sites/site_users.md). Se debe crear un usuario para la instalación y, a continuación, asignarle roles del sitio. Esto le permite otorgar permisos mínimos para toda la instalación a alguien que vaya a redactar y publicar una exposición virtual como un único sitio.

Los administradores gestionan y crean usuarios desde la sección Usuarios del panel de control administrativo principal (con el icono de la cabeza y los hombros). La sección Usuarios muestra el correo electrónico del usuario seguido de su nombre completo entre paréntesis, así como su rol y la fecha de creación de la cuenta.

Hay seis roles de usuario en Omeka S:

- **Administrador global**: privilegios completos de instalación.
...
- **Investigador**: solo tiene privilegios de búsqueda y lectura.

En segundo plano, los valores de los roles utilizan los siguientes slugs:
...
Un ejemplo de cómo se pueden gestionar los usuarios de su sitio:

- Un usuario **Global Admin** que instala, gestiona y actualiza los módulos y temas, y controla la configuración básica de la instalación y las utilidades del servidor.
- Un **Supervisor** que gestiona las cuentas de usuario en la instalación y los sitios.
- Uno o más **Editores** responsables del diseño y la gestión de elementos, conjuntos de elementos y plantillas de recursos.
- Miembros del personal **revisores**, responsables de añadir manualmente elementos digitalizados o recopilados y describirlos según las directrices institucionales, así como de editar las aportaciones de otros.
...
- Un **Rol** del menú desplegable (consulte más arriba los privilegios de los roles de usuario)
- Marque la casilla **Está activo** para confirmar que el usuario está activo. Un usuario inactivo no puede iniciar sesión.

También tiene la opción de personalizar otros ajustes; [consulte más abajo para obtener más detalles sobre la configuración de usuario](#user-settings).

A continuación, se envía al usuario un correo electrónico de notificación para que configure su cuenta y contraseña. El correo parecerá provenir de la dirección de correo electrónico configurada como la del administrador en la [configuración general de la instalación](settings.md#general). Cada correo caducará 14 días después de su envío. Si el correo caduca, un administrador puede configurar manualmente una contraseña temporal para el usuario y enviársela por correo electrónico.

> *¡Saludos!*
>
> *Se ha creado una cuenta para usted en Omeka en [sitio web]*
>
> *Su nombre de usuario es su correo electrónico: [correo electrónico]*
>
> *Haga clic en este enlace para establecer una contraseña y empezar a utilizar Omeka S: [enlace único del sitio web]*
>
> *Tu enlace de activación caducará el [fecha y hora]. Si no has completado el proceso de activación de usuario antes de que caduque el enlace, deberás solicitar otro correo electrónico de activación al administrador de tu sitio.*

!!! nota
  Si tienes problemas para enviar correos electrónicos desde tu instalación de Omeka S, consulta con tu proveedor de alojamiento. Algunas configuraciones de alojamiento pueden requerir que el nombre de dominio del correo electrónico del administrador coincida con el nombre de dominio de la instalación (si su dominio es `yourinstall.org`, el correo electrónico del administrador debe ser `user@yourinstall.org`). Omeka S utiliza la utilidad `sendmail` subyacente del servidor para enviar correos electrónicos; consulte la [sección Correo de la página de configuración](../configuration.md#mail) para obtener más información.

## Gestionar usuarios

Para gestionar un usuario existente, vaya a la sección Usuarios.

Todos los usuarios pueden editar su propia información mediante esta pantalla y pueden ver a otros usuarios de la instalación (y sus direcciones de correo electrónico). Solo los supervisores y el administrador global pueden editar a otros usuarios.
...
- **Configuración regional**: selecciona de un menú desplegable los idiomas disponibles. Esto cambiará el idioma de las interfaces de administración y pública de la instalación para ese usuario. (Esto depende de [cuántas cadenas de la interfaz se hayan traducido](../translateOmekaS.md). Las cadenas sin traducir se muestran por defecto en inglés.)
- **Plantilla de recursos predeterminada**: seleccione entre las plantillas existentes para que se cargue automáticamente una plantilla cada vez que el usuario cree un nuevo elemento. El elemento tendrá la clase asociada a la plantilla elegida.
- **Conjuntos de elementos predeterminados para los elementos**: seleccione entre los conjuntos de elementos disponibles para adjuntar automáticamente los nuevos elementos a los conjuntos cuando los cree el usuario. El [conjunto de elementos debe estar abierto a nuevos elementos](../content/item-sets.md#access-settings) para que se aplique esta configuración. El usuario podrá eliminar sus elementos de estos conjuntos de elementos. 
...
- **Columnas de navegación de administrador**: configura vistas personalizadas para los paneles de control administrativos de este usuario. Consulte a continuación para obtener más detalles.
...
Los usuarios pueden decidir qué columnas de información conforman sus vistas de navegación de recursos dentro de la interfaz de administración. Esto puede resultar útil, por ejemplo, si un usuario está auditando y mejorando un campo concreto para un gran número de elementos, o comparando información entre dos o más campos.

Las columnas se pueden añadir, eliminar y reordenar. Para añadir una columna, selecciónela en el menú desplegable disponible y, a continuación, pulse el botón con el signo más (+) situado a la derecha del menú.

La mayoría de los campos de metadatos se pueden añadir seleccionando «Valor» en el menú desplegable y, a continuación, eligiendo la propiedad de vocabulario concreta en el panel que se abre a la derecha. Puede tener tantas columnas de Valor como desee.
...
![Interfaz de columnas de navegación de administración en el área principal de la interfaz con el menú desplegable «Añadir una columna» para la columna de navegación de elementos abierta](adminfiles/users_settings_browsecolumns.png)
...
El campo de texto «Predeterminado» permite al usuario decidir qué información se muestra en una celda de la columna si no hay datos disponibles. Normalmente estas celdas están vacías, pero podría utilizar esto para mostrar, por ejemplo, «[null]» en su lugar, si estuviera buscando celdas que solo contengan espacios u otros caracteres invisibles.
...
#### Valores predeterminados de navegación de administrador
...
### Claves API
Utilice esta pestaña para generar una clave API para el usuario. Para generar una, debe proporcionar una etiqueta para la clave; esta puede ser una fecha o la finalidad de la clave. Guarde la página para generar la clave.

Para eliminar claves API existentes, marque la casilla «¿Eliminar?» en la fila de esa clave y haga clic en «Guardar».

![Pestaña API para el usuario Omeka Devs con una clave API generada pero oculta, y debajo de ella la opción de crear una nueva clave](adminfiles/users_apikey.png)
...
Para editar solo los usuarios cuyas casillas de verificación haya marcado, seleccione «Editar seleccionados» en el menú desplegable y, a continuación, haga clic en «Ir». Se le redirigirá a la página Edición masiva de usuarios.

![Página de edición masiva de usuarios](adminfiles/users_batch2.png)

En la parte derecha de la página se muestran los correos electrónicos de los usuarios que está editando y a los que se aplicarán los cambios que realice.
...
- Establecer actividad: establece los usuarios seleccionados como activos, inactivos o sin cambios.
- Eliminar de [permisos del sitio](../sites/site_users.md): selecciona de un menú desplegable los sitios de la instalación de Omeka S, o utiliza la opción «todos los sitios» para eliminar a los usuarios seleccionados de todos los sitios.
...
Si elige «Editar todo» en lugar de «Editar seleccionados», puede aplicar los cambios a todos los usuarios —excluyéndose a sí mismo— de la instalación de Omeka S. ¡Utilice esta función con precaución!

### Eliminación masiva de usuarios
Para eliminar varios usuarios a la vez, marque las casillas de verificación situadas junto a sus nombres en la tabla Usuarios.

En el menú desplegable situado justo encima de la columna de correo electrónico, seleccione «Eliminar seleccionados» y haga clic en «Ir».

![Una flecha roja señala el menú desplegable donde se muestra «Eliminar seleccionados»](adminfiles/users_batchdel1.png)

Esto abrirá un panel en el lado derecho de la pantalla con un mensaje que le indicará el número de usuarios que está a punto de eliminar. Esta acción **no se puede deshacer**.

Para eliminar definitivamente a estos usuarios, haga clic en el botón rojo «Confirmar eliminación». Para cancelar, haga clic en la «X» de la esquina superior derecha del panel.
...
Para eliminar permanentemente a todos los usuarios, debe marcar la casilla «¿Está seguro?» antes de hacer clic en el botón «Confirmar eliminación». Para cancelar, haga clic en la «X» situada en la esquina superior derecha del panel.
...
| | Editar/Eliminar  | Todos | Todos | Todos | No | Los propios | No |
...
1. A menudo, un administrador global, un supervisor o un editor creará sitios y luego asignará gestores y creadores de entre los usuarios (por ejemplo, una tarea de clase o exposiciones virtuales realizadas por miembros del personal). La propiedad de un sitio no se puede reasignar a otros usuarios, y el propietario de un sitio siempre tiene acceso de nivel de gestor a los sitios de su propiedad, incluso si se modifica su rol a nivel de instalación. Consulte la [página de usuarios del sitio](../sites/site_users.md) para obtener más información.
...
- Privilegios completos para módulos: instalar, activar, configurar y utilizar.
...
- Privilegios de solo lectura para los módulos en la pestaña «Admin > Módulos».
...
- No tiene acceso a módulos, ajustes de instalación ni tareas.
- Puede ver a otros usuarios de la instalación y sus direcciones de correo electrónico, pero no puede añadir usuarios.

**Revisor**

- Puede buscar, ver, crear y editar todos los elementos, conjuntos de elementos y archivos multimedia. Solo puede eliminar aquellos elementos, conjuntos de elementos y archivos multimedia de su propiedad.
...
- No tiene acceso a los sitios a menos que haya sido añadido como usuario específico de ese sitio.
...
- Privilegios de búsqueda y visualización para todos los elementos públicos, conjuntos de elementos, plantillas de recursos y medios.
- No puede ver objetos marcados como privados, a menos que el objeto sea un sitio para el que tenga permisos específicos.
- No tiene acceso a los módulos.
- Puede ver a otros usuarios de la instalación y sus direcciones de correo electrónico.

Recuerde que un usuario con cualquier rol puede [añadirse a un sitio en cualquier nivel (Visor, Creador, Administrador)](../sites/site_users.md). Esto determinará si, por ejemplo, un usuario con nivel de Investigador añadido a un sitio como Administrador puede ver información privada sobre ese sitio.
